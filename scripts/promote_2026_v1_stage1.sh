...
if [ $# -gt 1 ]; then
  echo "Usage: scripts/promote_2026_v1_stage1.sh [run-dir]" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$REPO_ROOT/repo/training/promote_checkpoint.py" \
  --run-dir "$RUN_DIR" \
  --destination-dir "$PROMOTION_DIR" \
  --model-id proteinmpnn-2026-v1-stage1
